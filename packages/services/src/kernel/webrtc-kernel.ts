...
} from '@jupyterlab/services/lib/kernel/future';
...
type ICreateSubshellRequestMsg = any;
type ICreateSubshellReplyMsg = any;
type IDeleteSubshellRequestMsg = any;
type IDeleteSubshellReplyMsg = any;
type IListSubshellRequestMsg = any;
type IListSubshellReplyMsg = any;
 ...
 */
export class WebRTCKernelConnection implements Kernel.IKernelConnection {
    /**
     * Construct a kernel object.
     */
    constructor(
        options: Kernel.IKernelConnection.IOptions & { subshellId: string | null }
    ) {
        ...
        console.log('constructor comm', options);
    ...
    clone(
        ...
        console.log('clone comm', options);
    ...
    private _sendMessage(msg: KernelMessage.IMessage, queue = true) {
        console.log('Sending message3', msg, this.connectionStatus);

        if (this.status === 'dead') {
            console.log('Kernel is dead');
        ...
        if (
            ...
            console.log('Sending kernel_info_request');
        ...
        if (queue && this._pendingMessages.length > 0) {
            ...
            console.log('Pending messages', this._pendingMessages);
            return;
        }
        ...
        if (
            ...
            console.log('Sending message2', msg);
            this._ws!.send(
                this.serverSettings.serializer.serialize(msg, this._ws!.protocol)
            );
        } else if (queue) {
    ...
    async requestKernelInfo(): Promise<KernelMessage.IInfoReplyMsg | undefined> {
        const msg = KernelMessage.createMessage<KernelMessage.IInfoRequestMsg>({
        ...
        const supportedFeatures = (reply.content as any).supported_features;
    ...
    requestComplete(
        ...
        const msg = KernelMessage.createMessage<KernelMessage.ICompleteRequestMsg>({
    ...
    requestInspect(
        ...
        const msg = KernelMessage.createMessage<KernelMessage.IInspectRequestMsg>({
    ...
    requestHistory(
        ...
        const msg = KernelMessage.createMessage<KernelMessage.IHistoryRequestMsg>({
    ...
    > {
        console.log('Request Execute', content);
    ...
    > {
        const msg = KernelMessage.createMessage<KernelMessage.IDebugRequestMsg>({
    ...
    requestCreateSubshell(
        content: ICreateSubshellRequestMsg['content'],
        disposeOnDone: boolean = true
    ): Kernel.IControlFuture<ICreateSubshellRequestMsg, ICreateSubshellReplyMsg> {
        if (!this.supportsSubshells) {
            throw new Error('Kernel subshells are not supported');
        }

        const msg = KernelMessage.createMessage<ICreateSubshellRequestMsg>({
    ...
    requestDeleteSubshell(
        content: IDeleteSubshellRequestMsg['content'],
        disposeOnDone: boolean = true
    ): Kernel.IControlFuture<IDeleteSubshellRequestMsg, IDeleteSubshellReplyMsg> {
        if (!this.supportsSubshells) {
            throw new Error('Kernel subshells are not supported');
        }

        const msg = KernelMessage.createMessage<IDeleteSubshellRequestMsg>({
     ...
     */
    requestListSubshell(
        content: IListSubshellRequestMsg['content'],
        disposeOnDone: boolean = true
    ): Kernel.IControlFuture<IListSubshellRequestMsg, IListSubshellReplyMsg> {
        if (!this.supportsSubshells) {
            throw new Error('Kernel subshells are not supported');
        }

        const msg = KernelMessage.createMessage<IListSubshellRequestMsg>({
        ...
        return this.sendControlMessage(
            ...
            true,
            disposeOnDone
        ) as Kernel.IControlFuture<IListSubshellRequestMsg, IListSubshellReplyMsg>;
    }
    ...
    requestIsComplete(
        ...
        const msg =
            KernelMessage.createMessage<KernelMessage.IIsCompleteRequestMsg>({
    ...
    requestCommInfo(
        ...
        const msg = KernelMessage.createMessage<KernelMessage.ICommInfoRequestMsg>({
    ...
    private _updateConnectionStatus(
        ...
        if (this.status !== 'dead') {
            if (connectionStatus === 'connected') {
                ...
                const timeoutHandle = setTimeout(sendPendingOnce, KERNEL_INFO_TIMEOUT);
            } else {
     ...
     */
    private _onWSMessage = (msg: KernelMessage.IMessage) => {
